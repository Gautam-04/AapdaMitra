...
function Dashboard() {
  ...
  const executeTimelineFetch = async () => {
    ...
    try {
      const apiResponse = await fetch('http://localhost:8000/v1/jaldibanao/timeline', {
        method: 'POST',
  ...
  const fetchDailyReport = async () => {
    ...
    setError('');
    try {
      const response = await fetch('http://localhost:8000/v1/jaldibanao/dailyreport');
      if (!response.ok) {
  ...
  //   if (!fromDate || !toDate) {
  //     setModalContent('Please provide both From and To dates.');
  //     setModalVisible(true);
  //     return;
  //   }

  //   try {
  //     const response = await fetch('http://localhost:8000/v1/jaldibanao/timelinereport', {
  //       method: 'POST',
  //       headers: { 'Content-Type': 'application/json' },
  ...
  //     setModalVisible(true);
  //   } catch (error) {
  //     setModalContent('Error fetching timeline report');
  //     setModalVisible(true);
  //   }
  // };
  ...
  return (
    <>
      <Header />
      <div style={{ padding: '40px', backgroundColor: '#f4f7f6', minHeight: 'calc(100vh - 200px)' }}>
      <h2
  style={{
    textAlign: 'center',
    marginBottom: '20px',
...
<Modal show={show} onHide={handleClose}>
  ...
  <Modal.Footer>
    ...
    <Button
      variant="primary"
      onClick={() => {
        const printableArea = document.getElementById('printableContent');
        if (printableArea) {
...
<Modal show={isPopupVisible} onHide={() => setPopupVisibility(false)}>
  ...
  <Modal.Footer>
    ...
    <Button variant="secondary" onClick={() => setPopupVisibility(false)}>
      Close
    </Button>
    <Button
      variant="primary"
